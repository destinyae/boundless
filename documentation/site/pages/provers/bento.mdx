...
The REST API provides an external interface to start / stop / monitor jobs and tasks within TaskDB. Bento is intended to be a drop in replacement for Bonsai, including being partially Bonsai API compatible. The [Bonsai API docs](https://api.bonsai.xyz/swagger-ui/) provide a good reference for the Bento REST API.
